  ...
  <div id="KQ" className="w3-content" 
      ...
      >
        ...
        >
          ...
          <tbody>
            <tr v-for="(item, index) in duLieuTim.slice(0, soLuongHienThi)" :key="index">
              <td>{{ index + 1 }}</td>
              <td>{{ item.maPhong }}</td>
              <td>{{ item.tenPhong }}</td>
              <td>{{ item.maDay }}</td>
...
export default {
  ...
  methods: {
    ...
    handleSumit (e) {
      e.preventDefault()
      if(this.duLieu.length >= this.duLieuTim.length){
        this.duLieuTim = this.duLieu
      }
      this.filterSearch(this.duLieuTim)
    },

    getTrangThaiSuDung (event) {
      this.trangThaiSuDung = event.target.value
    },

    getMaDay (event) {
      this.maDay = event.target.value
    },

    getMaLoaiPhong (event) {
      this.maLoaiPhong = event.target.value
    },

    getPhongNamNu (event) {
      this.phongNamNu = event.target.value
    },

    filterSearch (duLieu) {
      const tim = {
        trangThaiSuDung: this.trangThaiSuDung,
        maDay: this.maDay,
        maLoaiPhong: this.maLoaiPhong,
        phongNamNu: this.phongNamNu
      }
